...
from eth44.ecdsa import ECPointAffine, secp256k1
from eth44.ecdsa_base import Point
from eth44.utils import address_to_key_hash, bytes_to_str, rand_bytes
...
class PublicKey(PublicKeyBase):
    ...
    def from_signature(message, signature):
        ...
        pub_keys = bitcoin_curve.recover_public_key(
            msg, signature, signature.recovery_id
        )
...
class Signature(object):
    ...
    def to_der(self):
        ...
        der = (
            bytes([0x30, total_length - 2, 0x02, len(r)])
            + r
            + bytes([0x02, len(s)])
            + s
        )
...
class HDKey(object):
    ...
    def from_bytes(b):
        ...
        rv = None
        if (
            version == HDPrivateKey.MAINNET_VERSION
            or version == HDPrivateKey.TESTNET_VERSION
        ):
            if key_bytes[0] != 0:
            ...
            version == HDPublicKey.MAINNET_VERSION
            or version == HDPublicKey.TESTNET_VERSION
        ):
    ...
    def _serialize(self, testnet=False):
        ...
        key_bytes = (
            ...
            if isinstance(self, HDPublicKey)
            else b"\x00" + bytes(self._key)
        )
        return (
            version.to_bytes(length=4, byteorder="big")
            + bytes([self.depth])
...
class HDPrivateKey(HDKey, PrivateKeyBase):
    ...
    def from_parent(parent_key, i):
        ...
        if i & 0x80000000:
            hmac_data = (
                b"\x00" + bytes(parent_key._key) + i.to_bytes(length=4, byteorder="big")
            )
        else:
            hmac_data = parent_key.public_key.compressed_bytes + i.to_bytes(
    ...
    def __init__(
        self, key, chain_code, index, depth, parent_fingerprint=b"\x00\x00\x00\x00"
    ):
        if index < 0 or index > 0xFFFFFFFF:
            raise ValueError("index is out of range: 0 <= index <= 2**32 - 1")
...
class HDPublicKey(HDKey, PublicKeyBase):
    ...
    def __init__(
        self, x, y, chain_code, index, depth, parent_fingerprint=b"\x00\x00\x00\x00"
    ):
        key = PublicKey(x, y)
